...
export class RoundGeneratorService {
  ...
  generate(categories: CategoryId[], roundLength: number): Question[] {
    const length = normalizeRoundLength(roundLength);
    const perBand = length / 3;
    const all = this.bank.getAll().filter((q) => categories.includes(q.category));
  ...
  private pickForDifficulty(
    ...
    // Round-robin across categories for balance, then fill.
    ...
    while (picked.length < count && guard < count * 20) {
      ...
      for (const category of categories) {
        ...
        if (next) {
          picked.push(next);
          list.push(next); // allow reuse if bank is thin
