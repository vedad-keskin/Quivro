import type { Question } from '../../types';

export const moviesEasy: Question[] = [
  {
    id: 'movie-easy-001',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: {
      en: 'What type of animal is Mickey Mouse's pet Pluto?',
      bs: 'Koja životinja je Mikijev ljubimac Pluto?',
    },
    options: [
      { en: 'Dog', bs: 'Pas' },
      { en: 'Cat', bs: 'Mačka' },
      { en: 'Horse', bs: 'Konj' },
      { en: 'Rabbit', bs: 'Zec' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-002',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What shape is Harry Potter's forehead scar?',
              bs: 'Koji oblik ima ožiljak na Harry Potterovom čelu?' },
    options: [
      { en: 'Star', bs: 'Zvijezda' },
      { en: 'Heart', bs: 'Srce' },
      { en: 'Lightning bolt', bs: 'Munja' },
      { en: 'Circle', bs: 'Krug' },
    ],
    correctIndex: 2,
  },
  {
    id: 'movie-easy-003',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In Friends, the gang often hangs out at which café?',
              bs: 'U Friendsu, ekipa se često okuplja u kojem kafiću?' },
    options: [
      { en: 'Central Perk', bs: 'Central Perk' },
      { en: 'MacLaren's', bs: 'MacLaren's' },
      { en: 'Monk's Café', bs: 'Monk's Café' },
      { en: 'Luke's Diner', bs: 'Luke's Diner' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-004',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Elsa is a main character in which Disney film?',
              bs: 'Elsa je glavni lik u kojem Disney filmu?' },
    options: [
      { en: 'Moana', bs: 'Moana' },
      { en: 'Tangled', bs: 'Tangled' },
      { en: 'Frozen', bs: 'Frozen' },
      { en: 'Encanto', bs: 'Encanto' },
    ],
    correctIndex: 2,
  },
  {
    id: 'movie-easy-005',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Batman usually protects which city?',
              bs: 'Batman obično štiti koji grad?' },
    options: [
      { en: 'Metropolis', bs: 'Metropolis' },
      { en: 'Gotham', bs: 'Gotham' },
      { en: 'Star City', bs: 'Star City' },
      { en: 'Central City', bs: 'Central City' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-006',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'The Simpsons live in which town?',
              bs: 'Porodica Simpson živi u kojem gradu?' },
    options: [
      { en: 'Quahog', bs: 'Quahog' },
      { en: 'South Park', bs: 'South Park' },
      { en: 'Springfield', bs: 'Springfield' },
      { en: 'Pawnee', bs: 'Pawnee' },
    ],
    correctIndex: 2,
  },
  {
    id: 'movie-easy-007',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What did the Titanic hit that caused it to sink?',
              bs: 'U šta je Titanic udario zbog čega je potonuo?' },
    options: [
      { en: 'Whale', bs: 'Kit' },
      { en: 'Iceberg', bs: 'Ledenjak' },
      { en: 'Submarine', bs: 'Podmornica' },
      { en: 'Pirate ship', bs: 'Gusarski brod' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-008',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What weapon do Jedi use in Star Wars?',
              bs: 'Koje oružje koriste Jedi u Ratovima zvijezda?' },
    options: [
      { en: 'Lightsaber', bs: 'Svjetlosni mač' },
      { en: 'Bow', bs: 'Luk' },
      { en: 'Hammer', bs: 'Čekić' },
      { en: 'Trident', bs: 'Trozubac' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-009',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Shrek is what kind of creature?',
              bs: 'Kakvo je stvorenje Shrek?' },
    options: [
      { en: 'Dragon', bs: 'Zmaj' },
      { en: 'Giant', bs: 'Div' },
      { en: 'Ogre', bs: 'Oger' },
      { en: 'Wizard', bs: 'Čarobnjak' },
    ],
    correctIndex: 2,
  },
  {
    id: 'movie-easy-010',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What does the name "Digimon" stand for?',
              bs: 'Šta znači naziv "Digimon"?' },
    options: [
      { en: 'Digital Monsters', bs: 'Digitalna čudovišta' },
      { en: 'Digital Monkeys', bs: 'Digitalni majmuni' },
      { en: 'Digital Masters', bs: 'Digitalni majstori' },
      { en: 'Digital Machines', bs: 'Digitalne mašine' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-011',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What color are SpongeBob's pants?',
              bs: 'Koje su boje SpongeBobove hlače?' },
    options: [
      { en: 'Blue', bs: 'Plave' },
      { en: 'Brown', bs: 'Smeđe' },
      { en: 'Red', bs: 'Crvene' },
      { en: 'Green', bs: 'Zelene' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-012',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is Spider-Man's most well-known ability?',
              bs: 'Koja je najpoznatija sposobnost Spider-Mana?' },
    options: [
      { en: 'Flying', bs: 'Letenje' },
      { en: 'Invisibility', bs: 'Nevidljivost' },
      { en: 'Climbing walls', bs: 'Penjanje po zidovima' },
      { en: 'Breathing fire', bs: 'Bljuvanje vatre' },
    ],
    correctIndex: 2,
  },
  {
    id: 'movie-easy-013',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What type of fish is Nemo in Finding Nemo?',
              bs: 'Koja vrsta ribe je Nemo u filmu Potraga za Nemom?' },
    options: [
      { en: 'Shark', bs: 'Morski pas' },
      { en: 'Clownfish', bs: 'Riba klaun' },
      { en: 'Dolphin', bs: 'Dupin' },
      { en: 'Whale', bs: 'Kit' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-014',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who is the snowman in Frozen?',
              bs: 'Kako se zove snjegović u Frozen-u?' },
    options: [
      { en: 'Olaf', bs: 'Olaf' },
      { en: 'Sven', bs: 'Sven' },
      { en: 'Kristoff', bs: 'Kristoff' },
      { en: 'Hans', bs: 'Hans' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-015',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is Homer Simpson's favorite food?',
              bs: 'Koja je Homerova omiljena hrana?' },
    options: [
      { en: 'Sandwiches', bs: 'Sendviči' },
      { en: 'Doughnuts', bs: 'Krofne' },
      { en: 'Sushi', bs: 'Suši' },
      { en: 'Broccoli', bs: 'Brokula' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-016',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What type of toy is Woody in Toy Story?',
              bs: 'Koja vrsta igračke je Woody u filmu Toy Story?' },
    options: [
      { en: 'Cowboy', bs: 'Kauboj' },
      { en: 'Robot', bs: 'Robot' },
      { en: 'Dinosaur', bs: 'Dinosaurus' },
      { en: 'Alien', bs: 'Vanzemaljac' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-017',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What role does Buzz Lightyear play in Toy Story?',
              bs: 'Koju ulogu ima Buzz Lightyear u filmu Toy Story?' },
    options: [
      { en: 'Pirate', bs: 'Gusar' },
      { en: 'Space ranger', bs: 'Svemirski rendžer' },
      { en: 'Chef', bs: 'Kuhar' },
      { en: 'Doctor', bs: 'Doktor' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-018',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What animal is Simba in The Lion King?',
              bs: 'Kakva je životinja Simba u Kralju lavova?' },
    options: [
      { en: 'Tiger', bs: 'Tigar' },
      { en: 'Lion', bs: 'Lav' },
      { en: 'Bear', bs: 'Medvjed' },
      { en: 'Wolf', bs: 'Vuk' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-019',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In The Avengers, who can control the thunder?',
              bs: 'U Osvetnicima, ko može upravljati gromom?' },
    options: [
      { en: 'Loki', bs: 'Loki' },
      { en: 'Thor', bs: 'Thor' },
      { en: 'Hulk', bs: 'Hulk' },
      { en: 'Hawkeye', bs: 'Hawkeye' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-020',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of Vin Diesel's character in the Fast & Furious franchise?',
              bs: 'Kako se zove lik kojeg Vin Diesel glumi u filmovima Paklene ulice?' },
    options: [
      { en: 'Dominic Toretto', bs: 'Dominic Toretto' },
      { en: 'Brian O'Conner', bs: 'Brian O'Conner' },
      { en: 'Luke Hobbs', bs: 'Luke Hobbs' },
      { en: 'Han Seoul-Oh', bs: 'Han Seoul-Oh' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-021',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What fruit do the Minions love?',
              bs: 'Koje voće Minioni vole?' },
    options: [
      { en: 'Bananas', bs: 'Banane' },
      { en: 'Broccoli', bs: 'Brokulu' },
      { en: 'Pizza', bs: 'Pizzu' },
      { en: 'Ice cream', bs: 'Sladoled' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-022',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who lives in a pineapple under the sea?',
              bs: 'Ko živi u ananasu pod morem?' },
    options: [
      { en: 'Patrick', bs: 'Patrick' },
      { en: 'SpongeBob', bs: 'SpongeBob' },
      { en: 'Squidward', bs: 'Squidward' },
      { en: 'Mr. Krabs', bs: 'Mr. Krabs' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-023',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What type of students attend Hogwarts in Harry Potter?',
              bs: 'Kakvi učenici pohađaju Hogwarts u Harry Potteru?' },
    options: [
      { en: 'Wizards', bs: 'Čarobnjaci' },
      { en: 'Athletes', bs: 'Sportisti' },
      { en: 'Chefs', bs: 'Kuhari' },
      { en: 'Pilots', bs: 'Piloti' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-024',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of Elsa's sister in Frozen?',
              bs: 'Kako se zove Elsina sestra u Frozen?' },
    options: [
      { en: 'Moana', bs: 'Moana' },
      { en: 'Anna', bs: 'Anna' },
      { en: 'Belle', bs: 'Belle' },
      { en: 'Ariel', bs: 'Ariel' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-025',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In The Office (US), who is the boss at the start?',
              bs: 'U The Officeu (SAD), ko je šef na početku?' },
    options: [
      { en: 'Jim Halpert', bs: 'Jim Halpert' },
      { en: 'Dwight Schrute', bs: 'Dwight Schrute' },
      { en: 'Michael Scott', bs: 'Michael Scott' },
      { en: 'Pam Beesly', bs: 'Pam Beesly' },
    ],
    correctIndex: 2,
  },
  {
    id: 'movie-easy-026',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Which franchise is Pikachu the mascot of?',
              bs: 'Koje franšize je Pikachu maskota?' },
    options: [
      { en: 'Pokémon', bs: 'Pokémon' },
      { en: 'Star Wars', bs: 'Ratovi zvijezda' },
      { en: 'Friends', bs: 'Friends' },
      { en: 'Titanic', bs: 'Titanic' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-027',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In Aladdin, what does Aladdin find in the cave?',
              bs: 'U Aladdinu, šta Aladdin pronađe u pećini?' },
    options: [
      { en: 'A magic lamp', bs: 'Čarobnu lampu' },
      { en: 'A car', bs: 'Automobil' },
      { en: 'A crown', bs: 'Krunu' },
      { en: 'A sword', bs: 'Mač' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-028',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What color is the Hulk when he transforms?',
              bs: 'Koje je boje Hulk kad se pretvori?' },
    options: [
      { en: 'Blue', bs: 'Plave' },
      { en: 'Red', bs: 'Crvene' },
      { en: 'Green', bs: 'Zelene' },
      { en: 'Yellow', bs: 'Žute' },
    ],
    correctIndex: 2,
  },
  {
    id: 'movie-easy-029',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In Cinderella, what does she leave behind at the ball?',
              bs: 'U Pepeljugi, šta ostavi na balu?' },
    options: [
      { en: 'A glove', bs: 'Rukavicu' },
      { en: 'A glass slipper', bs: 'Staklenu cipelu' },
      { en: 'A necklace', bs: 'Ogrlicu' },
      { en: 'A hat', bs: 'Šešir' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-030',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Which actor played Brian O'Conner in the Fast & Furious films?',
              bs: 'Koji glumac je glumio Briana O'Connera u filmovima Paklene ulice?' },
    options: [
      { en: 'Vin Diesel', bs: 'Vin Diesel' },
      { en: 'Dwayne Johnson', bs: 'Dwayne Johnson' },
      { en: 'Paul Walker', bs: 'Paul Walker' },
      { en: 'Jason Statham', bs: 'Jason Statham' },
    ],
    correctIndex: 2,
  },
  {
    id: 'movie-easy-031',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What creatures fill the park in Jurassic Park?',
              bs: 'Kakva stvorenja se nalaze u Jurassic Parku?' },
    options: [
      { en: 'Dinosaurs', bs: 'Dinosaurusi' },
      { en: 'Robots', bs: 'Roboti' },
      { en: 'Aliens', bs: 'Vanzemaljci' },
      { en: 'Pirates', bs: 'Gusari' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-032',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the blue fish in Finding Nemo?',
              bs: 'Kako se zove plava riba u Potrazi za Nemom?' },
    options: [
      { en: 'Dory', bs: 'Dora' },
      { en: 'Nemo', bs: 'Nemo' },
      { en: 'Marlin', bs: 'Marlin' },
      { en: 'Bruce', bs: 'Bruce' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-033',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is Darth Vader most famous for wearing in Star Wars?',
              bs: 'Po čemu je Darth Vader najprepoznatljiviji u Ratovima zvijezda?' },
    options: [
      { en: 'Black helmet', bs: 'Crna kaciga' },
      { en: 'Yellow hat', bs: 'Žuti šešir' },
      { en: 'Red cape', bs: 'Crveni ogrtač' },
      { en: 'Green mask', bs: 'Zelena maska' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-034',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What do players use to battle in the Yu-Gi-Oh anime?',
              bs: 'Čime se igrači bore u Yu-Gi-Oh animeu?' },
    options: [
      { en: 'Cards', bs: 'Karte' },
      { en: 'Dice', bs: 'Kockice' },
      { en: 'Spinning tops', bs: 'Čigre' },
      { en: 'Marbles', bs: 'Klikeri' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-035',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In Moana, which demigod helps Moana on her journey?',
              bs: 'U Moani, ko pomaže Moani na putovanju?' },
    options: [
      { en: 'Maui', bs: 'Maui' },
      { en: 'Hercules', bs: 'Hercules' },
      { en: 'Genie', bs: 'Duh' },
      { en: 'Olaf', bs: 'Olaf' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-036',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What animal is Scooby-Doo?',
              bs: 'Kakva je životinja Scooby-Doo?' },
    options: [
      { en: 'Cat', bs: 'Mačka' },
      { en: 'Dog', bs: 'Pas' },
      { en: 'Mouse', bs: 'Miš' },
      { en: 'Bird', bs: 'Ptica' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-037',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What mythical creature is Ariel in The Little Mermaid?',
              bs: 'Kakvo mitsko stvorenje je Ariel u Maloj sireni?' },
    options: [
      { en: 'Mermaid', bs: 'Sirena' },
      { en: 'Fairy', bs: 'Vila' },
      { en: 'Witch', bs: 'Vještica' },
      { en: 'Pirate', bs: 'Gusarica' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-038',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What does Captain America carry into battle?',
              bs: 'Šta Captain America nosi u bitku?' },
    options: [
      { en: 'Shield', bs: 'Štit' },
      { en: 'Hammer', bs: 'Čekić' },
      { en: 'Bow', bs: 'Luk' },
      { en: 'Wand', bs: 'Štapić' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-039',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who helps Gru carry out his plans in Despicable Me?',
              bs: 'Ko pomaže Gruu u izvršavanju planova u filmu Despicable Me?' },
    options: [
      { en: 'Minions', bs: 'Minioni' },
      { en: 'Avengers', bs: 'Osvetnici' },
      { en: 'Jedi', bs: 'Jedi' },
      { en: 'Pokémon', bs: 'Pokémoni' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-040',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of Bart Simpson's saxophone-playing sister?',
              bs: 'Kako se zove Bartova sestra koja svira saksofon?' },
    options: [
      { en: 'Maggie', bs: 'Maggie' },
      { en: 'Lisa', bs: 'Lisa' },
      { en: 'Marge', bs: 'Marge' },
      { en: 'Patty', bs: 'Patty' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-041',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What does Carl use to make his house fly in the movie Up?',
              bs: 'Čime Carl podiže svoju kuću u filmu Up?' },
    options: [
      { en: 'Balloons', bs: 'Baloni' },
      { en: 'Rockets', bs: 'Rakete' },
      { en: 'Helicopters', bs: 'Helikopteri' },
      { en: 'Kites', bs: 'Zmajevi' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-042',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who is the main hero in The Mandalorian?',
              bs: 'Ko je glavni heroj u The Mandalorianu?' },
    options: [
      { en: 'Luke Skywalker', bs: 'Luke Skywalker' },
      { en: 'Din Djarin', bs: 'Din Djarin' },
      { en: 'Han Solo', bs: 'Han Solo' },
      { en: 'Obi-Wan', bs: 'Obi-Wan' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-043',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What tabletop game do the kids frequently play in Stranger Things?',
              bs: 'Koju društvenu igru djeca često igraju u seriji Stranger Things?' },
    options: [
      { en: 'Chess', bs: 'Šah' },
      { en: 'Dungeons & Dragons', bs: 'Dungeons & Dragons' },
      { en: 'Poker', bs: 'Poker' },
      { en: 'Golf', bs: 'Golf' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-044',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the green ogre's best friend?',
              bs: 'Kako se zove najbolji prijatelj zelenog ogra?' },
    options: [
      { en: 'Donkey', bs: 'Magare' },
      { en: 'Prince Charming', bs: 'Princ Šarmer' },
      { en: 'Fiona', bs: 'Fiona' },
      { en: 'Farquaad', bs: 'Farquaad' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-045',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In a Disney film about a cursed castle, which resident does Belle come to love?',
              bs: 'U Disneyjevom filmu o začaranom dvorcu, u koga se Belle zaljubi?' },
    options: [
      { en: 'The Beast', bs: 'Zvijer' },
      { en: 'Gaston', bs: 'Gaston' },
      { en: 'Lumière', bs: 'Lumière' },
      { en: 'Maurice', bs: 'Maurice' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-046',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: {
      en: 'What color are Mickey Mouse's iconic shorts?',
      bs: 'Koje su boje Mikijeve prepoznatljive kratke hlače?',
    },
    options: [
      { en: 'Blue', bs: 'Plave' },
      { en: 'Red', bs: 'Crvene' },
      { en: 'Green', bs: 'Zelene' },
      { en: 'Yellow', bs: 'Žute' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-047',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What was Walter White's profession before the events of Breaking Bad?',
              bs: 'Koji je bio posao Waltera Whitea prije događaja u Breaking Badu?' },
    options: [
      { en: 'Chef', bs: 'Kuhar' },
      { en: 'Teacher', bs: 'Učitelj' },
      { en: 'Pilot', bs: 'Pilot' },
      { en: 'Lawyer', bs: 'Advokat' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-048',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who is the princess with long magical hair in Tangled?',
              bs: 'Koja je princeza s dugom čarobnom kosom u Tangled?' },
    options: [
      { en: 'Rapunzel', bs: 'Rapunzel' },
      { en: 'Ariel', bs: 'Ariel' },
      { en: 'Jasmine', bs: 'Jasmine' },
      { en: 'Tiana', bs: 'Tiana' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-049',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What makes the Parr family special in The Incredibles?',
              bs: 'Šta čini porodicu Parr posebnom u filmu The Incredibles?' },
    options: [
      { en: 'Superpowers', bs: 'Supermoći' },
      { en: 'Magical wands', bs: 'Čarobni štapići' },
      { en: 'Great wealth', bs: 'Veliko bogatstvo' },
      { en: 'Advanced technology', bs: 'Napredna tehnologija' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-050',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is Baby Yoda's real name in The Mandalorian?',
              bs: 'Kako se zove Baby Yoda u The Mandalorianu?' },
    options: [
      { en: 'Grogu', bs: 'Grogu' },
      { en: 'Yoda', bs: 'Yoda' },
      { en: 'Anakin', bs: 'Anakin' },
      { en: 'Rey', bs: 'Rey' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-051',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What type of animal is Judy Hopps in Zootopia?',
              bs: 'Kakva je životinja Judy Hopps u Zootopiji?' },
    options: [
      { en: 'Fox', bs: 'Lisica' },
      { en: 'Rabbit', bs: 'Zec' },
      { en: 'Lion', bs: 'Lav' },
      { en: 'Bear', bs: 'Medvjed' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-052',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What does Superman usually wear on his chest?',
              bs: 'Šta Superman obično ima na prsima?' },
    options: [
      { en: 'An S logo', bs: 'Logo S' },
      { en: 'A star', bs: 'Zvijezdu' },
      { en: 'A heart', bs: 'Srce' },
      { en: 'A number', bs: 'Broj' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-053',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Where does most of the story of Inside Out take place?',
              bs: 'Gdje se najveći dio priče u filmu Inside Out odvija?' },
    options: [
      { en: 'Inside a mind', bs: 'Unutar uma' },
      { en: 'Inside a school', bs: 'Unutar škole' },
      { en: 'Inside a car', bs: 'Unutar auta' },
      { en: 'Inside a garden', bs: 'Unutar vrta' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-054',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who is the villain with a red lightsaber in many Star Wars scenes?',
              bs: 'Ko je negativac s crvenim svjetlosnim mačem u mnogim scenama Ratova zvijezda?' },
    options: [
      { en: 'Darth Vader', bs: 'Darth Vader' },
      { en: 'Yoda', bs: 'Yoda' },
      { en: 'Leia', bs: 'Leia' },
      { en: 'R2-D2', bs: 'R2-D2' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-055',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What type of toy do characters battle with in the Beyblade anime?',
              bs: 'Kojom vrstom igračke se likovi bore u Beyblade animeu?' },
    options: [
      { en: 'Spinning tops', bs: 'Čigre' },
      { en: 'Trading cards', bs: 'Kartice' },
      { en: 'Dice', bs: 'Kockice' },
      { en: 'Action figures', bs: 'Akcione figure' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-056',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the boy wizard with glasses?',
              bs: 'Kako se zove dječak čarobnjak s naočalama?' },
    options: [
      { en: 'Ron Weasley', bs: 'Ron Weasley' },
      { en: 'Harry Potter', bs: 'Harry Potter' },
      { en: 'Draco Malfoy', bs: 'Draco Malfoy' },
      { en: 'Neville Longbottom', bs: 'Neville Longbottom' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-057',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What kind of home does Mirabel's family live in, in Encanto?',
              bs: 'U kakvoj kući živi Mirabelina porodica u Encantu?' },
    options: [
      { en: 'Magical house', bs: 'Čarobna kuća' },
      { en: 'Spaceship', bs: 'Svemirski brod' },
      { en: 'Submarine', bs: 'Podmornica' },
      { en: 'Treehouse', bs: 'Kućica na drvetu' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-058',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What fruit does Snow White eat that puts her to sleep?',
              bs: 'Koje voće Snjeguljica pojede i zaspi?' },
    options: [
      { en: 'Banana', bs: 'Bananu' },
      { en: 'Apple', bs: 'Jabuku' },
      { en: 'Orange', bs: 'Narandžu' },
      { en: 'Grape', bs: 'Grožđe' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-059',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who is Simba's father in The Lion King?',
              bs: 'Ko je Simbin otac u Kralju lavova?' },
    options: [
      { en: 'Scar', bs: 'Scar' },
      { en: 'Mufasa', bs: 'Mufasa' },
      { en: 'Timon', bs: 'Timon' },
      { en: 'Pumbaa', bs: 'Pumbaa' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-060',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the yellow sponge's best starfish friend?',
              bs: 'Kako se zove najbolji prijatelj žute spužve, morska zvijezda?' },
    options: [
      { en: 'Patrick', bs: 'Patrick' },
      { en: 'Squidward', bs: 'Squidward' },
      { en: 'Plankton', bs: 'Plankton' },
      { en: 'Gary', bs: 'Gary' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-061',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What planet do the Na'vi live on in Avatar?',
              bs: 'Na kojoj planeti žive Na'vi u Avataru?' },
    options: [
      { en: 'Earth', bs: 'Zemlja' },
      { en: 'Pandora', bs: 'Pandora' },
      { en: 'Mars', bs: 'Mars' },
      { en: 'Moon', bs: 'Mjesec' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-062',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who is the red racing car in Cars?',
              bs: 'Ko je crveni trkaći auto u Carsu?' },
    options: [
      { en: 'Mater', bs: 'Šlep' },
      { en: 'Lightning McQueen', bs: 'Munjeviti Jurić' },
      { en: 'Doc Hudson', bs: 'Doc Hudson' },
      { en: 'Sally', bs: 'Sali' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-063',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is Ross Geller's profession in Friends?',
              bs: 'Koji je Rossov posao u seriji Friends?' },
    options: [
      { en: 'Chef', bs: 'Kuhar' },
      { en: 'Paleontologist', bs: 'Paleontolog' },
      { en: 'Pilot', bs: 'Pilot' },
      { en: 'Lawyer', bs: 'Advokat' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-064',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In E.T. the Extra-Terrestrial, what does E.T. famously want to phone?',
              bs: 'U filmu E.T., šta E.T. poznato želi nazvati?' },
    options: [
      { en: 'Home', bs: 'Kuću' },
      { en: 'School', bs: 'Školu' },
      { en: 'The police', bs: 'Policiju' },
      { en: 'A spaceship', bs: 'Svemirski brod' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-065',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In Home Alone, Kevin sets traps for two…',
              bs: 'U Sam u kući, Kevin postavlja zamke dvojici…' },
    options: [
      { en: 'Burglars', bs: 'Provalnika' },
      { en: 'Teachers', bs: 'Učitelja' },
      { en: 'Aliens', bs: 'Vanzemaljaca' },
      { en: 'Pirates', bs: 'Gusara' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-066',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What animal is Pumbaa in The Lion King?',
              bs: 'Kakva je životinja Pumbaa u Kralju lavova?' },
    options: [
      { en: 'Meerkat', bs: 'Mungos' },
      { en: 'Warthog', bs: 'Bradavičasta svinja' },
      { en: 'Lion', bs: 'Lav' },
      { en: 'Elephant', bs: 'Slon' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-067',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What special ability does Barry Allen have in The Flash?',
              bs: 'Koju posebnu sposobnost ima Barry Allen u seriji The Flash?' },
    options: [
      { en: 'Flight', bs: 'Letenje' },
      { en: 'Super speed', bs: 'Super brzina' },
      { en: 'Invisibility', bs: 'Nevidljivost' },
      { en: 'Underwater breathing', bs: 'Disanje pod vodom' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-068',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the magical blue wish-granter in Aladdin?',
              bs: 'Kako se zove čarobni plavi lik koji ispunjava želje u Aladdinu?' },
    options: [
      { en: 'Genie', bs: 'Genie' },
      { en: 'Jafar', bs: 'Jafar' },
      { en: 'Abu', bs: 'Abu' },
      { en: 'Iago', bs: 'Iago' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-069',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What device do children use to evolve their partners in Digimon?',
              bs: 'Koji uređaj djeca koriste za evoluciju partnera u Digimon-u?' },
    options: [
      { en: 'Digivice', bs: 'Digivice' },
      { en: 'Duel Disk', bs: 'Duel Disk' },
      { en: 'Pokédex', bs: 'Pokédex' },
      { en: 'Bey Launcher', bs: 'Bey Launcher' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-070',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who is Batman's main sidekick?',
              bs: 'Ko je Batmanov glavni pomoćnik?' },
    options: [
      { en: 'Robin', bs: 'Robin' },
      { en: 'Joker', bs: 'Joker' },
      { en: 'Penguin', bs: 'Penguin' },
      { en: 'Riddler', bs: 'Riddler' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-071',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who do Mike and Sulley scare in Monsters, Inc.?',
              bs: 'Koga Mike i Sulley plaše u filmu Monsters, Inc.?' },
    options: [
      { en: 'Adults', bs: 'Odrasle' },
      { en: 'Children', bs: 'Djecu' },
      { en: 'Animals', bs: 'Životinje' },
      { en: 'Robots', bs: 'Robote' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-072',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the boy who never grows up?',
              bs: 'Kako se zove dječak koji nikad ne odraste?' },
    options: [
      { en: 'Peter Pan', bs: 'Peter Pan' },
      { en: 'Pinocchio', bs: 'Pinocchio' },
      { en: 'Aladdin', bs: 'Aladdin' },
      { en: 'Simba', bs: 'Simba' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-073',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the Iron Throne made of in Game of Thrones?',
              bs: 'Od čega je napravljeno Željezno prijestolje u Igri prijestolja?' },
    options: [
      { en: 'Gold', bs: 'Zlato' },
      { en: 'Swords', bs: 'Mačevi' },
      { en: 'Wood', bs: 'Drvo' },
      { en: 'Ice', bs: 'Led' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-074',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What color is Pikachu?',
              bs: 'Koje je boje Pikachu?' },
    options: [
      { en: 'Blue', bs: 'Plave' },
      { en: 'Yellow', bs: 'Žute' },
      { en: 'Green', bs: 'Zelene' },
      { en: 'Red', bs: 'Crvene' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-075',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Where does Dorothy want to go in The Wizard of Oz?',
              bs: 'Kuda Dorothy želi ići u Čarobnjaku iz Oza?' },
    options: [
      { en: 'Home', bs: 'Kući' },
      { en: 'To space', bs: 'U svemir' },
      { en: 'To school', bs: 'U školu' },
      { en: 'To the beach', bs: 'Na plažu' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-076',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What type of vehicles are the Fast & Furious films mainly centered around?',
              bs: 'Oko koje vrste vozila se vrte filmovi Paklene ulice?' },
    options: [
      { en: 'Cars', bs: 'Automobili' },
      { en: 'Motorcycles', bs: 'Motocikli' },
      { en: 'Airplanes', bs: 'Avioni' },
      { en: 'Boats', bs: 'Brodovi' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-077',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Which fictional nation is T'Challa king of in Black Panther?',
              bs: 'Kojom izmišljenom državom vlada T'Challa u Black Pantheru?' },
    options: [
      { en: 'Asgard', bs: 'Asgard' },
      { en: 'Wakanda', bs: 'Wakanda' },
      { en: 'Gotham', bs: 'Gotham' },
      { en: 'Atlantis', bs: 'Atlantis' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-078',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is Donald Duck famous for wearing?',
              bs: 'Po čemu je Donald Duck poznat što nosi?' },
    options: [
      { en: 'A sailor shirt', bs: 'Mornarsku majicu' },
      { en: 'A cowboy hat', bs: 'Kaubojski šešir' },
      { en: 'A cape', bs: 'Ogrtač' },
      { en: 'Armor', bs: 'Oklop' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-079',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What kind of creature does Hiccup befriend in How to Train Your Dragon?',
              bs: 'Kakvo stvorenje Štucko upozna u filmu Kako dresirati zmaja?' },
    options: [
      { en: 'Dragon', bs: 'Zmaj' },
      { en: 'Lion', bs: 'Lav' },
      { en: 'Robot', bs: 'Robot' },
      { en: 'Pirate', bs: 'Gusar' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-080',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What gives the Gauls their superhuman strength in the Astérix films?',
              bs: 'Šta daje Galima nadljudsku snagu u filmovima o Asterixu?' },
    options: [
      { en: 'A magic potion', bs: 'Čarobni napitak' },
      { en: 'A magic sword', bs: 'Čarobni mač' },
      { en: 'A magic ring', bs: 'Čarobni prsten' },
      { en: 'A magic shield', bs: 'Čarobni štit' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-081',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What weapon is Katniss Everdeen skilled with in The Hunger Games?',
              bs: 'Kojim oružjem je Katniss Everdeen vješta u Igrama gladi?' },
    options: [
      { en: 'Bow', bs: 'Luk' },
      { en: 'Hammer', bs: 'Čekić' },
      { en: 'Wand', bs: 'Štapić' },
      { en: 'Shield', bs: 'Štit' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-082',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the blue alien family friend of Lilo?',
              bs: 'Kako se zove plavi vanzemaljac koji je Lilin prijatelj?' },
    options: [
      { en: 'Stitch', bs: 'Stitch' },
      { en: 'Yoda', bs: 'Yoda' },
      { en: 'Grogu', bs: 'Grogu' },
      { en: 'ET', bs: 'ET' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-083',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the main character in the Yu-Gi-Oh anime?',
              bs: 'Kako se zove glavni lik u Yu-Gi-Oh animeu?' },
    options: [
      { en: 'Yugi Muto', bs: 'Yugi Muto' },
      { en: 'Seto Kaiba', bs: 'Seto Kaiba' },
      { en: 'Joey Wheeler', bs: 'Joey Wheeler' },
      { en: 'Maximillion Pegasus', bs: 'Maximillion Pegasus' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-084',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What does Pinocchio's nose do when he lies?',
              bs: 'Šta se desi Pinocchiovom nosu kad laže?' },
    options: [
      { en: 'It grows', bs: 'Raste' },
      { en: 'It shrinks', bs: 'Smanjuje se' },
      { en: 'It turns blue', bs: 'Postane plav' },
      { en: 'It disappears', bs: 'Nestane' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-085',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What does Miguel dream of doing in the movie Coco?',
              bs: 'O čemu Miguel sanja u filmu Coco?' },
    options: [
      { en: 'Playing music', bs: 'Sviranju muzike' },
      { en: 'Playing soccer', bs: 'Igranju fudbala' },
      { en: 'Cooking', bs: 'Kuhanju' },
      { en: 'Racing', bs: 'Utrkivanju' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-086',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who is the villain with green hair and a purple suit in Batman stories?',
              bs: 'Ko je negativac sa zelenom kosom i ljubičastim odijelom u Batman pričama?' },
    options: [
      { en: 'Joker', bs: 'Joker' },
      { en: 'Bane', bs: 'Bane' },
      { en: 'Two-Face', bs: 'Two-Face' },
      { en: 'Scarecrow', bs: 'Scarecrow' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-087',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the main character in the original Beyblade anime?',
              bs: 'Kako se zove glavni lik u originalnom Beyblade animeu?' },
    options: [
      { en: 'Tyson Granger', bs: 'Tyson Granger' },
      { en: 'Kai Hiwatari', bs: 'Kai Hiwatari' },
      { en: 'Ray Kon', bs: 'Ray Kon' },
      { en: 'Max Tate', bs: 'Max Tate' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-088',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the princess who sleeps for a long time?',
              bs: 'Kako se zove princeza koja dugo spava?' },
    options: [
      { en: 'Sleeping Beauty', bs: 'Trnoružica' },
      { en: 'Cinderella', bs: 'Pepeljuga' },
      { en: 'Belle', bs: 'Belle' },
      { en: 'Jasmine', bs: 'Jasmine' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-089',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What choice is Neo offered in The Matrix?',
              bs: 'Koji izbor je Neu ponuđen u filmu Matrix?' },
    options: [
      { en: 'Red pill or blue pill', bs: 'Crvena ili plava pilula' },
      { en: 'Sword or shield', bs: 'Mač ili štit' },
      { en: 'Car or bike', bs: 'Auto ili bicikl' },
      { en: 'Hat or cape', bs: 'Šešir ili ogrtač' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-090',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What animal is Timon in The Lion King?',
              bs: 'Kakva je životinja Timon u Kralju lavova?' },
    options: [
      { en: 'Warthog', bs: 'Bradavičasta svinja' },
      { en: 'Meerkat', bs: 'Merkat' },
      { en: 'Lion', bs: 'Lav' },
      { en: 'Hyena', bs: 'Hijena' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-091',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What type of competitions do players face in Squid Game?',
              bs: 'U kakvim takmičenjima se igrači suočavaju u Igri lignje?' },
    options: [
      { en: 'Deadly children's games', bs: 'Smrtonosne dječije igre' },
      { en: 'Cooking contests', bs: 'Kulinarska takmičenja' },
      { en: 'Singing shows', bs: 'Pjevačke emisije' },
      { en: 'Car races', bs: 'Auto trke' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-092',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: {
      en: 'What is the name of Peter Pan's fairy companion?',
      bs: 'Kako se zove vilinska prijateljica Petra Pana?',
    },
    options: [
      { en: 'Tinker Bell', bs: 'Zvončica' },
      { en: 'Fairy Godmother', bs: 'Vila kuma' },
      { en: 'Elsa', bs: 'Elsa' },
      { en: 'Wendy', bs: 'Wendy' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-093',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What type of animal is Remy in Ratatouille?',
              bs: 'Kakva je životinja Remy u filmu Ratatouille?' },
    options: [
      { en: 'Cat', bs: 'Mačka' },
      { en: 'Rat', bs: 'Pacov' },
      { en: 'Dog', bs: 'Pas' },
      { en: 'Bird', bs: 'Ptica' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-094',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'Who is the main hero of the Marvel movies with a metal suit?',
              bs: 'Ko je glavni heroj Marvel filmova s metalnim odijelom?' },
    options: [
      { en: 'Iron Man', bs: 'Iron Man' },
      { en: 'Thor', bs: 'Thor' },
      { en: 'Hulk', bs: 'Hulk' },
      { en: 'Hawkeye', bs: 'Hawkeye' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-095',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of Astérix's large and strong best friend?',
              bs: 'Kako se zove Asterixov veliki i snažni najbolji prijatelj?' },
    options: [
      { en: 'Obelix', bs: 'Obelix' },
      { en: 'Getafix', bs: 'Getafix' },
      { en: 'Dogmatix', bs: 'Dogmatix' },
      { en: 'Cacofonix', bs: 'Cacofonix' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-096',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What does Wall-E collect and clean?',
              bs: 'Šta Wall-E sakuplja i čisti?' },
    options: [
      { en: 'Trash', bs: 'Smeće' },
      { en: 'Stars', bs: 'Zvijezde' },
      { en: 'Cars', bs: 'Automobile' },
      { en: 'Books', bs: 'Knjige' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-097',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'How do the children enter the magical world in The Chronicles of Narnia?',
              bs: 'Kako djeca ulaze u čarobni svijet u Hronikama Narnije?' },
    options: [
      { en: 'Through a wardrobe', bs: 'Kroz ormar' },
      { en: 'Through a window', bs: 'Kroz prozor' },
      { en: 'Through a mirror', bs: 'Kroz ogledalo' },
      { en: 'Through a painting', bs: 'Kroz sliku' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-098',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the green one-eyed monster in Monsters, Inc.?',
              bs: 'Kako se zove zeleni čudovište s jednim okom u Monsters, Inc.?' },
    options: [
      { en: 'Mike', bs: 'Mike' },
      { en: 'Sulley', bs: 'Sulley' },
      { en: 'Randall', bs: 'Randall' },
      { en: 'Boo', bs: 'Boo' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-099',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What type of warrior is Diana in Wonder Woman?',
              bs: 'Kakva je ratnica Diana u filmu Wonder Woman?' },
    options: [
      { en: 'Amazon warrior', bs: 'Amazonka ratnica' },
      { en: 'Chef', bs: 'Kuharica' },
      { en: 'Pilot', bs: 'Pilotkinja' },
      { en: 'Teacher', bs: 'Učiteljica' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-100',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'What is the name of the talking teapot in Beauty and the Beast?',
              bs: 'Kako se zove čajnik koji priča u Ljepotici i zvijeri?' },
    options: [
      { en: 'Mrs. Potts', bs: 'Mrs. Potts' },
      { en: 'Lumière', bs: 'Lumière' },
      { en: 'Cogsworth', bs: 'Cogsworth' },
      { en: 'Chip', bs: 'Chip' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-101',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In Arrow, which city does Oliver Queen protect?',
              bs: 'U seriji Arrow, koji grad štiti Oliver Queen?' },
    options: [
      { en: 'Gotham', bs: 'Gotham' },
      { en: 'Star City', bs: 'Star City' },
      { en: 'Metropolis', bs: 'Metropolis' },
      { en: 'Central City', bs: 'Central City' },
    ],
    correctIndex: 1,
  },
  {
    id: 'movie-easy-102',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In The Flash, which city is Barry Allen from?',
              bs: 'U seriji The Flash, iz kojeg grada je Barry Allen?' },
    options: [
      { en: 'Star City', bs: 'Star City' },
      { en: 'Gotham', bs: 'Gotham' },
      { en: 'Central City', bs: 'Central City' },
      { en: 'Coast City', bs: 'Coast City' },
    ],
    correctIndex: 2,
  },
  {
    id: 'movie-easy-103',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'The Lord of the Rings films are mainly set in which fictional world?',
              bs: 'Filmovi Gospodar prstenova uglavnom su smješteni u koji izmišljeni svijet?' },
    options: [
      { en: 'Middle-earth', bs: 'Srednja zemlja' },
      { en: 'Westeros', bs: 'Westeros' },
      { en: 'Narnia', bs: 'Narnija' },
      { en: 'Hogwarts', bs: 'Hogwarts' },
    ],
    correctIndex: 0,
  },
  {
    id: 'movie-easy-104',
    type: 'mcq',
    category: 'movies',
    difficulty: 'easy',
    prompt: { en: 'In Ace Ventura: Pet Detective, who plays the title character?',
              bs: 'U Ace Ventura: Pet Detective, ko glumi naslovnog lika?' },
    options: [
      { en: 'Jim Carrey', bs: 'Jim Carrey' },
      { en: 'Eddie Murphy', bs: 'Eddie Murphy' },
      { en: 'Will Ferrell', bs: 'Will Ferrell' },
      { en: 'Adam Sandler', bs: 'Adam Sandler' },
    ],
    correctIndex: 0,
  },
];
